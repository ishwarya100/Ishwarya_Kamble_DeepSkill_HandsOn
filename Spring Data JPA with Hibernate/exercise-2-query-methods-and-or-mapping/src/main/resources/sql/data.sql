-- ============================================================
-- Sample data for orm-learn
-- Country data below is the exact reference list supplied in
-- Hands on 5 (File 2). All other sample data (department,
-- skill, employee, stock, quiz) was not supplied as a separate
-- file in the hand-out (it referenced external payroll.sql /
-- stock-data.csv / quiz.mwb files), so representative sample
-- rows are provided here to keep every hands-on exercise
-- runnable out of the box. Stock and quiz rows below match the
-- exact sample values and expected results shown in the document.
-- ============================================================
...
insert into employee_skill (es_em_id, es_sk_id) values (6, 4);
insert into employee_skill (es_em_id, es_sk_id) values (8, 3);

-- ============================================================
-- Hands on 2 (File 2): Stock data
-- Facebook (FB) data for September 2019, matches the expected
-- result shown in the document for the "FB in September 2019" query
-- ============================================================
...
-- Facebook rows with the three highest trading volumes overall
...
-- Google (GOOGL) rows with closing price greater than 1250
...
-- Netflix (NFLX) rows, three lowest trading volumes plus a couple of higher
-- volume rows so the "three lowest" query below has a meaningful result
...
insert into stock (st_code, st_date, st_open, st_close, st_volume) values ('NFLX', '2019-06-03', 340.20, 349.60, 25000000);
insert into stock (st_code, st_date, st_open, st_close, st_volume) values ('NFLX', '2019-07-01', 355.10, 361.20, 30000000);
